...
class NaverWebtoon : NaverComicBase("webtoon") {
    ...
    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request {
        ...
        val isStandardGenre = listOf("DAILY", "COMIC", "FANTASY", "ACTION", "DRAMA", "PURE", "SENSIBILITY", "THRILL", "HISTORICAL", "SPORTS").contains(genreParam)
        ...
        val url = baseUrl.toHttpUrl().newBuilder().apply {
            if (dayParam == "finished") {
                ...
                if (isStandardGenre) {
                    ...
                    // [해결] 세부 태그(#먼치킨 등) 검색 시 500 에러 방지를 위해 검색 API 사용하되, 정렬 파라미터는 제외합니다.
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        ...
        // 일반 키워드 검색 또는 세부 태그 우회 검색 시 searchList 처리
        if (jsonObject.containsKey("searchList")) {
            ...
            // [해결] 태그 검색 시 전체 페이지를 로드하여 타치요미 내부에서 사용자가 선택한 기준으로 로컬 정렬 수행
...
internal val genreList = listOf(
    FilterOption("전체", ""),
    FilterOption("로맨스", "PURE"), // 👈 '순정'이 '로맨스'로 명칭 변경됨
    ...
    FilterOption("연도별웹툰", "연도별웹툰"),
    FilterOption("브랜드웹툰", "브랜드웹툰"),
...
internal val challengeSortList = listOf(
    FilterOption("조회순", "VIEW"),
    FilterOption("업데이트순", "UPDATE"),
    FilterOption("별점순", "starScore"), // [해결] 베도/도전만화 서버 규칙에 맞춘 파라미터 복구
)
